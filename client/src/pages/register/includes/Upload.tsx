...
import RenewalApplicationType from "./ApplicationType";
...
export default function Upload({
  ...
  const generateBatchNumber = () => {
    ...
    for (
      let batchNumber = 1;
      batchNumber <= CURRENT_BATCH_GROUP;
      batchNumber++
    ) {
      ...
      <div className="col-6">
        ...
        <div className="form-floating my-3 rounded-3">
          <input
            ...
            value={amountPaid}
            onChange={(e) => updateData({ amountPaid: Number(e.target.value) })}
          />
          <label>Amount Paid</label>
          {errors.amountPaid && (
            <span className="text-danger">{errors.amountPaid.message}</span>
          )}
        </div>
        <div>
          <div className="mb-3">
          ...
          <div className="mb-3">
            ...
            <input
              ...
              id="receipt"
              {...register("receipt")}
              onChange={(e) => updateData({ receipt: e.target.files?.[0] })}
            />
            {receipt.name && (
              <div>
      ...
      <div className="col-6">
        <div className="form-floating mb-3 rounded-3">
          ...
          {errors.dateIdValidity && (
            <span className="text-danger">{errors.dateIdValidity.message}</span>
          )}
        </div>
        <div className="form-floating mb-3 rounded-3">
          <input
            ...
            onChange={(e) => updateData({ transactionDetails: e.target.value })}
          />
          <label>Bank Name, Location, Date and Time of Deposit/ Transfer</label>
        ...
        <div className="mb-3">
          ...
          {errors.nationalCert && (
            <>
              <br />
              <span className="text-danger">{errors.nationalCert.message}</span>
            </>
          )}
        </div>
        <div className="mb-3">
